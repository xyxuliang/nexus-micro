...
import (
	...
	"github.com/xyxuliang/nexus-micro/examples/etcd-demo/payment-service/internal/handler"
...
func Register(r *gin.Engine, ctx *svc.ServiceContext) {
	...
	api := r.Group("/api/v1")
	{
		api.POST("/payments", h.CreatePayment(ctx))
		api.GET("/payments/:id", h.GetPayment(ctx))
	}
}
